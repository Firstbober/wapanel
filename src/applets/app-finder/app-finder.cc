	...
	action_bar::action_bar(action_bar::config conf, logout_box::config logout_conf)
		...
		if (cmp_config.user_manager_cmd != "") {
			...
			gtk_container_add(GTK_CONTAINER(m_user_account_setting),
							  GTK_WIDGET(gtk_image_new_from_pixbuf(utils::ic::get_icon("user-info-symbolic", 24))));
		...
		if (cmp_config.settings_cmd != "") {
			...
			gtk_container_add(
				GTK_CONTAINER(m_system_settings),
				GTK_WIDGET(gtk_image_new_from_pixbuf(utils::ic::get_icon("preferences-system-symbolic", 24))));
		...
		if (cmp_config.file_manager_cmd != "") {
			...
			gtk_container_add(
				GTK_CONTAINER(m_file_manager),
				GTK_WIDGET(gtk_image_new_from_pixbuf(utils::ic::get_icon("system-file-manager-symbolic", 24))));
